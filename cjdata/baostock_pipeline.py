...
def _require_baostock() -> None:
    if bs is None:  # pragma: no cover
        raise RuntimeError(
            "baostock is not installed. Install baostock to use this feature."
        )
...
class BaostockPipeline:
    ...
    def download_daily_for_codes(
        ...
        adjustflags: Sequence[str] = ("1", "3"),
    ) -> int:
        """Download daily K-line data for given codes.

        Parameters
        ----------
        adjustflags : sequence of str
            Baostock adjustflag values to download.
            ``"1"`` = 后复权 (back-adjusted), ``"2"`` = 前复权 (forward-adjusted),
            ``"3"`` = 未复权 (no adjustment).  Defaults to ``("1", "3")``.
        """
        ...
        with _baostock_session() as session:
            ...
            for code in iterable:
                for adjustflag in adjustflags:
                    rows = self._download_single_daily(
                        session, code, start, end, adjustflag
                    )
                    if not rows:
                        continue
                    spec = UpsertSpec(
    ...
    def download_dupont_for_codes(
        ...
        with _baostock_session() as session:
            for code in codes:
                total += self._download_single_dupont(
                    session, code, start_year, start_quarter
                )
        self.conn.commit()
        return total
    ...
    def _download_single_daily(
        ...
        frame["adjustflag"] = (
            pd.to_numeric(frame["adjustflag"], errors="coerce")
            .fillna(int(adjustflag))
            .astype(int)
        )
    ...
    def _download_single_dupont(
        ...
        for year in range(start_year, current.year + 1):
            ...
            for quarter in range(first_quarter, last_quarter + 1):
                try:
                    rs = session.query_dupont_data(
                        code=bs_code, year=year, quarter=quarter
                    )
                except Exception as exc:  # pragma: no cover
                    self.logger.warning(
                        "dupont download failed for %s %sQ%s: %s",
                        stock_code,
                        year,
                        quarter,
                        exc,
